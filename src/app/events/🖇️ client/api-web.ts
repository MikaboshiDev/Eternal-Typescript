...
export default new Event('messageCreate', async (message) => {
  ...
  if (msg) return logWithLabel('info', 'Message already exists in the database');
  ...
  if (data) return logWithLabel('info', `Message already exists in the database the second time ${message.id}`);
  ...
  msgModel
    .save()
    .then(() => {
      logWithLabel('info', 'Message saved to the database in the day ' + new Date().toLocaleDateString());
    })
    .catch((err) => {
