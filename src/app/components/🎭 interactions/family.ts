...
module.exports = {
  ...
  async execute(client: any, message: Message, args: string[], prefix: any) {
    try {
        ...
        case 'adopt':
          {
            ...
            if (!target || target.id === message.author.id)
              return message.channel.send({
                content: [
                  `${emojis.error} You must mention a member to adopt them`,
                  `> \`${prefix}adopt [user]\``,
                ].join('\n'),
              });
        ...
        case 'propose':
          {
            ...
            if (target.user.bot)
              return message.channel.send({
                content: [
                  `${emojis.error} You cannot propose to a bot :pensive:`,
                  `> \`${prefix}propose [user]\``,
                ].join('\n'),
              });
      ...
      logWithLabel('error', `The following error ocurred in the family command: ${err}`);
      message.channel.send({
        content: [`${emojis.error} The following error ocurred:`, `\`\`\`${err}\`\`\``].join('\n'),
      });
    }
  },
};
