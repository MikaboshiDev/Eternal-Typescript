...
export default new Command(
  new SlashCommandBuilder()
    ...
    .addNumberOption((option) =>
      option.setName('amount').setDescription('The amount to pay the user').setRequired(true).setMaxValue(1000).setMinValue(1)
    ),
  async (client, interaction) => {
    ...
    if (member?.bot || member?.id === interaction.user.id)
      return await interaction.reply({
        embeds: [new EmbedBuilder().setDescription('You cant send money to yourself or a bot!').setColor('Red')],
        ephemeral: true,
      });

    if (amount > userBalance.balance)
      return await interaction.reply({
        embeds: [new EmbedBuilder().setDescription("You don't have enough money in your balance to pay that user.").setColor('Red')],
      });
    ...
    await user.findOneAndUpdate({ _id: userBalance._id }, { balance: await toFixedNumber(userBalance.balance - amount) });
    ...
    await interaction.reply({
      embeds: [new EmbedBuilder().setDescription(`You have successfully transferred $${amount} to ${user}!`).setColor('Green')],
      ephemeral: true,
    });
    ...
    await member_send?.send({
      embeds: [
        new EmbedBuilder()
          .setDescription(
            `You have received a total of ${amount} from ${interaction.user}! This amount has been deposited to your balance and you total now is $${
