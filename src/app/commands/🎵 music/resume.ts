...
export default new Command(
  new SlashCommandBuilder().setName('resume').setDescription('🎵 Continue playing the song'),
  async (client, interaction) => {
    ...
    if (player.isPaused) {
      ...
      return interaction.reply({
        content: [
          `${emojis.correct} The song has been resumed`,
          `consider using the command \`pause\` to pause playback`,
        ].join('\n'),
        ephemeral: true,
      });
    } else {
      return interaction.reply({
        content: [
          `${emojis.error} The song is already playing`,
          `consider using the command \`pause\` to pause playback`,
        ].join('\n'),
        ephemeral: true,
      });
